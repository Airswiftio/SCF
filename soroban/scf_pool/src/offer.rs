use crate::storage_types::{DataKey, OFFER_LIFETIME_THRESHOLD, OFFER_BUMP_AMOUNT, Offer};
...
pub fn read_offer(e: &Env, offer_id: i128) -> Option<Offer> {
    ...
    if let Some(offer) = e.storage().persistent().get::<DataKey, Offer>(&key) {
        e.storage().persistent().bump(&key, OFFER_LIFETIME_THRESHOLD, OFFER_BUMP_AMOUNT);
        Some(offer)
    } else {
        None
    }
